...
class ChaincodeInitForm extends Component {
    ...
    handleParamCreate = () => {
        ...
        const newParams = params.map(param => ({ ...param, text: '-'})).concat({id: this.id++, data: '', text: '+'});
    ...
    handleParams = (param) => {
        if ( param.id === this.id -1 ) {// last param
            this.handleParamCreate();
        } else { this.handleParamRemove(param.id) }
    };

    handleParamChange = (e) => {
        const { request } = this.state;
        const { params } = this.state.request;
        const newParam = params.map( param => e.target.name === param.id.toString() ? ({...param, data: e.target.value }) : param );
        this.setState({ request: {
                ...request,
                params: newParam
            }})
       ...
       return (
           <div className={"card "+ classes.container}>
               <div className="card-header" align="center">
                   <h3>Instantiate Chaincode ({chaincodeInfo.name} ver:{chaincodeInfo.version}) </h3>
               </div>
               <div className={"card-body"}>
                   <form className={classes.container1} onSubmit={this.handleSubmit}>
                       ...
                       >
                           { peerList.map(peer => {
                           return (<MenuItem key={peer.requests} value={peer}>{peer.server_hostname}</MenuItem>);
                       })};
                       </TextField>
                       <TextField
                           ...
                           request.params.map((param, index) => (
                               <div className={classes.paramBlock}>
                                   <TextField
                                       ...
                                       margin="normal"
                                   />
                                       <Button className={classes.paramBtn} onClick={ () => this.handleParams(param) }>{ param.text }</Button>
                               </div>
                           ))
                       }
