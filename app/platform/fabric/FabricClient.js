...
class FabricClient {
  ...
  async initialize(client_config, persistence) {
    ...
    if (
      ...
      let username = explorer_const.DEFAULT_USERNAME;
      if (
        explorer_config &&
  ...
  async getRegisteredUser(client_config, username) {
    try {
      ...
      if (user && user.isEnrolled()) {
        ...
        var admins = explorer_config.admins;
        ...
        let secret = await caClient.register(
          {
            ...
            affiliation:
              userOrg.toLowerCase() + explorer_config.user.affiliation
          },
          adminUserObj
        );
