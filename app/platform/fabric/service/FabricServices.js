...
class FabricServices {
    ...
    async insertNewChannel(client, channel, block, genesis_block_hash) {
        ...
        if (block.data && block.data.data.length > 0 && block.data.data[0]) {
            ...
            await this.persistance.getCrudService().saveChannel(channel_row);
        }
    }
    ...
    async insertNewPeer(peer, genesis_block_hash, client) {
        ...
        await this.persistance.getCrudService().savePeer(peer_row);
        let channel_peer_row = {
            "peerid": host_port[0],
            "channelid": genesis_block_hash
        }
        console.log(" PEER Channel:>>>>>>>>>>>>>>>>>>>>>" + JSON.stringify(channel_peer_row));
        await this.persistance.getCrudService().savePeerChannelRef(channel_peer_row);
    ...
    async insertNewOrderers(orderer, genesis_block_hash) {
        ...
        await this.persistance.getCrudService().savePeer(orderer_row);
        let channel_orderer_row = {
            "peerid": orderer.host,
            "channelid": genesis_block_hash
        }
        console.log(" ORDERER CHANNEL :>>>>>>>>>>>>>>>>>>>>>" + JSON.stringify(channel_orderer_row));
        await this.persistance.getCrudService().savePeerChannelRef(channel_orderer_row);
    }
    ...
    async insertNewChaincode(client, channel, genesis_block_hash) {
        ...
        for (let chaincode of chaincodes.chaincodes) {
            ...
            await this.persistance.getCrudService().saveChaincode(chaincode_row);
    ...
    async insertNewChaincodePeerRef(peer) {
        ...
        for (var chaincode of peer.chaincodes) {
            ...
            await this.persistance.getCrudService().saveChaincodPeerRef(chaincode_peer_row);
        }
    ...
    async synchBlocks(client_name) {
        ...
        console.log('Oh snap! signature_header' + JSON.stringify(channels));
        ...
        for (let channel of channels.channels) {
            ...
            let results = await this.persistance.getMetricService().findMissingBlockNumber(channel_name, blockHeight);
    ...
    async processBlockEvent(block) {
        ...
        let channelConfig = await this.persistance.getCrudService().getChannelConfig(channel_name);

        if (!channelConfig) {
            let client = this.platform.getChannelClient(channel_name);
            await saveChannelFromBlock(client, channel_name);
            channelConfig = await this.persistance.getCrudService().getChannelConfig(channel_name);
        }
        ...
        if (channelConfig) {
            ...
            for (let i = 0; i < txLen; i++) {
                ...
                let res = await this.persistance.getCrudService().saveTransaction(transaction_row);
            }
            let status = await this.persistance.getCrudService().saveBlock(block_row);
        } else {
        ...
        // typeString = "CONFIG" and type = 1 is current peer joined to new channel 

        // "typeString":"ENDORSER_TRANSACTION" and "chaincode_id":lscc is new chaincode deoployed in the current peer

        // "typeString":"ENDORSER_TRANSACTION" and "chaincode_id": mycc new block created in ledger
