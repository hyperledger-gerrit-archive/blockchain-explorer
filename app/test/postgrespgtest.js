...
describe('Test explorerpg.sql for DDL statements syntax verification',  function ()
{
 it('should read the file explorerpg.sql for ddl statements ', function (readdone)
 {
    ...
    instream = fs.createReadStream('../persistence/postgreSQL/db/explorerpg.sql'),
    outstream = new (require('stream'))(),
    rl = readline.createInterface(instream, outstream);
    rl.on('line', function (line) {
        if((line.toUpperCase().startsWith('CREATE') || line.toUpperCase().startsWith('DROP')
            || line.toUpperCase().startsWith('ALTER') || line.toUpperCase().startsWith('GRANT')) && line.endsWith(';'))
            {
        ...
        }else if ((line.toUpperCase().startsWith('CREATE') || line.toUpperCase().startsWith('DROP')
        || line.toUpperCase().startsWith('ALTER') || line.toUpperCase().startsWith('GRANT')) && !line.endsWith(';'))
        {
            sb.append(line);
            isMergeline=true;
...
it('should execute statements successfully in  explorerpg.sql file ', function (testdone){
    ...
    for(let i=0;i<newList.size();i++)
    {
        options.tests[i] = client => {
                ...
                return client.query(newList.get(i))
                .then(() => {
