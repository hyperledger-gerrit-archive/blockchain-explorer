...
describe('GET /api/transaction/:channel/:id', () => {
    ...
    it('should return currentchannel ', (done) => {
        ...
        request.get(`${base}` + '/api/transaction/mychannel/1752ce850935e0547e78b5396f64162a09c595f9ecc514f25afe48b52fa4d840', (err, body) => {
            body = JSON.parse(body);
            body.should.include.keys(
                'status','row');
            body.status.should.eql(200);
            body.row.should.include.keys(
                'id','channelname','txhash','blockid','chaincodename','status','createdt','creator_msp_id','endorser_msp_id','type','chaincode_id','write_set','read_set')
            body.row.id.should.eql(10)
            body.row.status.should.eql(200);
            body.row.channelname.should.eql('mychannel');
            body.row.txhash.should.eql('1752ce850935e0547e78b5396f64162a09c595f9ecc514f25afe48b52fa4d840')
            ...
            for(let i=0;i<body.row.write_set.length;i++){
                body.row.write_set[i].should.include.keys(
                    'chaincode','set'
                )
            }
            for(let i=0;i<body.row.read_set.length;i++){
                body.row.read_set[i].should.include.keys(
                    'chaincode','set'
                )
            }
