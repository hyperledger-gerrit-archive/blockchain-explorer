...
do
sudo docker exec -e "CORE_LOGGING_LEVEL=INFO" -e "CORE_PEER_LOCALMSPID=Org1MSP" -e "CORE_PEER_MSPCONFIGPATH=$CORE_PEER_MSPCONFIGPATH" -e "CORE_PEER_ADDRESS=$CORE_PEER_ADDRESS" cli peer chaincode invoke -o orderer.example.com:7050  --tls $CORE_PEER_TLS_ENABLED --cafile \
$CORE_PEER_TLS_ROOTCERT_FILE -C $CHANNEL_NAME -n mycc -c '{"Args":["invoke","a","b","3000"]}'  
n=`expr $n + 1`
done
exit 0
